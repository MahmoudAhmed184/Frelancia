...
- unsafe direct controls are rendered only when `WXT_ENABLE_UNSAFE_DIRECT_AI=true`.
...
| `appendText(parent, text)` inner                     | Appends text node to generated help content.                                                 | element, text                                  | `void`                                    | Used by unsafe direct-mode help rendering.                                                                      |
| `createFormGroup(labelText, field, helpText?)` inner | Creates generated direct-mode form group.                                                    | label, field, help text                        | element                                   | Used only for unsafe direct controls.                                                                           |
| `renderUnsafeDirectAiControls()` inner               | Adds direct provider/model/key/system prompt controls when the unsafe build flag is enabled. | none                                           | `void`                                    | Adds direct option, warning help text, and fields; skipped in normal builds.                                    |
...
| `ensureDirectAiPermissionBeforeSave(settings)` inner | Requests selected provider host permission for unsafe direct mode.                           | settings                                       | `Promise<boolean>`                        | Uses dynamic import of direct-AI permission helper; blocks save when permission is denied.                      |
...
| `exportBackup(button?)` inner                        | Downloads JSON backup.                                                                       | optional button                                | `Promise<void>`                           | Creates Blob URL and temporary anchor named `rasid_backup_<date>.json`; revokes URL.                        |
